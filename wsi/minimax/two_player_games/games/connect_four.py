...
class ConnectFourState(State):
    ...
    def count_pieces(self, column:int,  player: Player) -> int:
        return self.fields[column].count(player)

    def count_checks(self, player: Player) -> [int]:
        checks = []

        for col in self.fields:  # vertical
            counter = None
            for row in col:
                if row == player:
        ...
        for row in range(len(self.fields[0])):   # horizontal
            counter = None
            for col in range(len(self.fields)):
                if self.fields[col][row] == player:
                    counter = counter + 1 if counter else 1
                else:
                    if counter:
                        checks.append(counter)
        ...
        # iteratote from rigth to left diagonaly
        for i in range(len(self.fields) + len(self.fields[0]) - 1):
            counter = None
            for j in range(max(0, i - len(self.fields) + 1), min(i + 1, len(self.fields[0]))):
                if self.fields[i - j][j] == player:
                    counter = counter + 1 if counter else 1
                else:
                    if counter:
                        checks.append(counter)
        ...
        # iterate from left to right diagonally
        for i in range(-len(self.fields[0]) + 1, len(self.fields)):
            counter = None
            for j in range(max(0, i), min(len(self.fields[0]), len(self.fields) + i)):
                if self.fields[j - i][j] == player:
                    counter = counter + 1 if counter else 1
                else:
                    if counter:
                        checks.append(counter)
    ...
    def __str__(self) -> str:
        # Define color codes for each player
        players_in_order = sorted(self.get_players(), key=lambda player: player.char)
        player_colors = dict(zip([player.char for player in players_in_order], ['94m', '91m']))

        print_field = lambda field: f'[\033[{player_colors.get(field.char, " ")}{field.char}\033[0m]' if field else '[  ]'
        transposed = zip(*self.fields)
        text = '\n'.join(
            reversed([''.join(
                print_field(field) for field in row) for row in transposed])
        )

        return f'Current player: \033[{player_colors.get(self._current_player.char, "")}{self._current_player.char}\033[0m\n{text}'
